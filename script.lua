--[[
    xrex zob - Emote GUI
    Modified from 7yd7 Emote Script
    - Tombol KOTAK (sudut = 0)
    - Title bar "xrex zob" + tombol minimize & resize
    - Bagian EMOT dan ANIMASI tetap UTUH & terpisah
    
    CARA PAKAI: Run script ini langsung dari executor lo
]]
...
DiscordCorner.CornerRadius = UDim.new(0, 0)
...
local hudEditorBtn = SettingsLib:Create("TextButton", {
    ...
    TextSize = 11
}, { SettingsLib:Create("UICorner", {CornerRadius = UDim.new(0, 0)}) })
...
State.enterCustomAnimationEditor = function(category, animName)
    ...
    getgenv().Notify({ Title = "7yd7 | Animation Editor", Content = "ðŸ–±ï¸ Select an animation from the wheel to set for " .. animName, Duration = 5 })
end
...
SettingsLib.AddIconButton(CustomAnimMgtContainer, "78317476576895", function()
    ...
    imp.MouseButton1Click:Connect(function()
        ...
        if s and type(d) == "table" then
            ...
            if refreshCustomAnimationState then refreshCustomAnimationState(false) end
            popup:Destroy()
            getgenv().Notify({ Title = "7yd7 | Animation", Content = "âœ… Imported custom animations", Duration = 3 })
        else
            getgenv().Notify({ Title = "Error", Content = "Invalid JSON", Duration = 3 })
        end
    end)
...
SettingsLib.AddIconButton(EmotePageMgtContainer, "78317476576895", function() 
    ...
    imp.MouseButton1Click:Connect(function()
        ...
        if s and type(d) == "table" and d.Type == "EmotePageSet" and type(d.Data) == "table" then
            ...
            popup:Destroy()
            getgenv().Notify({ Title = "7yd7 | Page", Content = "âœ… Imported Emote page", Duration = 3 })
        else
...
SettingsLib.AddIconButton(AnimPageMgtContainer, "108445456753346", function() 
    ...
    Save.MouseButton1Click:Connect(function()
        if In.Text ~= "" and not State.AnimationPages.Sets[In.Text] then
            ...
            if State.AnimationPageDropdown then State.AnimationPageDropdown.Button.Text = State.currentAnimationPageName .. "  â–¼" end
            popup:Destroy()
        end
    end)
    Cancel.MouseButton1Click:Connect(function() popup:Destroy() end)
...
SettingsLib.AddIconButton(AnimPageMgtContainer, "71829270056766", function()
    if State.currentAnimationPageName ~= "Default" then
        ...
        if State.AnimationPageDropdown then
            ...
            State.AnimationPageDropdown.Button.Text = "Default  â–¼"
        end
...
SettingsLib.AddIconButton(AnimPageMgtContainer, "117761881427472", function()
    ...
    Save.MouseButton1Click:Connect(function()
        if In.Text ~= "" and not State.AnimationPages.Sets[In.Text] then
            ...
            if State.AnimationPageDropdown then
                State.AnimationPageDropdown.Refresh(GetAnimationPageNames())
                State.AnimationPageDropdown.Button.Text = State.currentAnimationPageName .. "  â–¼"
            end
            popup:Destroy()
        end
    end)
...
SettingsLib.AddIconButton(AnimPageMgtContainer, "78317476576895", function()
    ...
    imp.MouseButton1Click:Connect(function()
        ...
        if s and type(d) == "table" and d.Type == "AnimationPageSet" and type(d.Data) == "table" then
            ...
            popup:Destroy()
            getgenv().Notify({ Title = "7yd7 | Page", Content = "âœ… Imported Animation page", Duration = 3 })
        else
...
function createGUIElements()
    ...
    UICorner_Left.CornerRadius = UDim.new(0, 0)
    UICorner_Left.Parent = UI._1left
    
    local UICorner_Right = Instance.new("UICorner")
    UICorner_Right.CornerRadius = UDim.new(0, 0)
...
UICorner_5.Parent = UI.Changepage
    ...
    -- ===== xrex zob: TITLE BAR + MINIMIZE =====
    local xrzScreenGui = CoreGui:FindFirstChild("XRexZobOverlay")
    if xrzScreenGui then xrzScreenGui:Destroy() end
    xrzScreenGui = Instance.new("ScreenGui")
    xrzScreenGui.Name = "XRexZobOverlay"
    xrzScreenGui.ResetOnSpawn = false
    xrzScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    xrzScreenGui.Parent = CoreGui

    local xrzTitle = Instance.new("Frame")
    xrzTitle.Name = "XRexZobTitle"
    xrzTitle.Size = UDim2.new(0, 230, 0, 26)
    xrzTitle.Position = UDim2.new(0.127, 0, 0, 0)
    xrzTitle.BackgroundColor3 = Color3.fromRGB(20, 20, 24)
    xrzTitle.BorderSizePixel = 0
    xrzTitle.ZIndex = 500
    xrzTitle.Parent = xrzScreenGui

    local xrzLabel = Instance.new("TextLabel")
    xrzLabel.Size = UDim2.new(1, -60, 1, 0)
    xrzLabel.Position = UDim2.new(0, 8, 0, 0)
    xrzLabel.BackgroundTransparency = 1
    xrzLabel.Text = "xrex zob"
    xrzLabel.TextColor3 = Color3.fromRGB(0, 255, 150)
    xrzLabel.Font = Enum.Font.GothamBold
    xrzLabel.TextSize = 13
    xrzLabel.TextXAlignment = Enum.TextXAlignment.Left
    xrzLabel.ZIndex = 501
    xrzLabel.Parent = xrzTitle

    -- Tombol resize (biru)
    local xrzSizeBtn = Instance.new("TextButton")
    xrzSizeBtn.Size = UDim2.new(0, 26, 0, 22)
    xrzSizeBtn.Position = UDim2.new(1, -56, 0, 2)
    xrzSizeBtn.BackgroundColor3 = Color3.fromRGB(60, 80, 200)
    xrzSizeBtn.BorderSizePixel = 0
    xrzSizeBtn.Text = "âŠ¡"
    xrzSizeBtn.TextColor3 = Color3.new(1, 1, 1)
    xrzSizeBtn.Font = Enum.Font.GothamBold
    xrzSizeBtn.TextSize = 13
    xrzSizeBtn.ZIndex = 502
    xrzSizeBtn.Parent = xrzTitle

    -- Tombol minimize (merah)
    local xrzMinBtn = Instance.new("TextButton")
    xrzMinBtn.Size = UDim2.new(0, 26, 0, 22)
    xrzMinBtn.Position = UDim2.new(1, -28, 0, 2)
    xrzMinBtn.BackgroundColor3 = Color3.fromRGB(220, 50, 50)
    xrzMinBtn.BorderSizePixel = 0
    xrzMinBtn.Text = "â€”"
    xrzMinBtn.TextColor3 = Color3.new(1, 1, 1)
    xrzMinBtn.Font = Enum.Font.GothamBold
    xrzMinBtn.TextSize = 14
    xrzMinBtn.ZIndex = 502
    xrzMinBtn.Parent = xrzTitle

    -- Mini bar (muncul waktu di-minimize)
    local xrzMiniBar = Instance.new("Frame")
    xrzMiniBar.Name = "XRexZobMini"
    xrzMiniBar.Size = UDim2.new(0, 130, 0, 26)
    xrzMiniBar.Position = UDim2.new(0.127, 0, 0, 0)
    xrzMiniBar.BackgroundColor3 = Color3.fromRGB(20, 20, 24)
    xrzMiniBar.BorderSizePixel = 0
    xrzMiniBar.Visible = false
    xrzMiniBar.ZIndex = 500
    xrzMiniBar.Parent = xrzScreenGui

    local xrzMiniLabel = Instance.new("TextLabel")
    xrzMiniLabel.Size = UDim2.new(1, -30, 1, 0)
    xrzMiniLabel.Position = UDim2.new(0, 8, 0, 0)
    xrzMiniLabel.BackgroundTransparency = 1
    xrzMiniLabel.Text = "xrex zob"
    xrzMiniLabel.TextColor3 = Color3.fromRGB(0, 255, 150)
    xrzMiniLabel.Font = Enum.Font.GothamBold
    xrzMiniLabel.TextSize = 12
    xrzMiniLabel.TextXAlignment = Enum.TextXAlignment.Left
    xrzMiniLabel.ZIndex = 501
    xrzMiniLabel.Parent = xrzMiniBar

    local xrzRestoreBtn = Instance.new("TextButton")
    xrzRestoreBtn.Size = UDim2.new(0, 26, 0, 22)
    xrzRestoreBtn.Position = UDim2.new(1, -28, 0, 2)
    xrzRestoreBtn.BackgroundColor3 = Color3.fromRGB(0, 180, 100)
    xrzRestoreBtn.BorderSizePixel = 0
    xrzRestoreBtn.Text = "â–²"
    xrzRestoreBtn.TextColor3 = Color3.new(1, 1, 1)
    xrzRestoreBtn.Font = Enum.Font.GothamBold
    xrzRestoreBtn.TextSize = 12
    xrzRestoreBtn.ZIndex = 502
    xrzRestoreBtn.Parent = xrzMiniBar

    -- Drag title bar
    local xrzDragging, xrzDragStart, xrzDragPos = false, nil, nil
    xrzTitle.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 then
            xrzDragging = true; xrzDragStart = inp.Position; xrzDragPos = xrzTitle.Position
        end
    end)
    xrzMiniBar.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 then
            xrzDragging = true; xrzDragStart = inp.Position; xrzDragPos = xrzTitle.Position
        end
    end)
    UserInputService.InputChanged:Connect(function(inp)
        if xrzDragging and inp.UserInputType == Enum.UserInputType.MouseMovement then
            local d = inp.Position - xrzDragStart
            xrzTitle.Position = UDim2.new(xrzDragPos.X.Scale, xrzDragPos.X.Offset + d.X, xrzDragPos.Y.Scale, xrzDragPos.Y.Offset + d.Y)
            xrzMiniBar.Position = xrzTitle.Position
        end
    end)
    UserInputService.InputEnded:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 then xrzDragging = false end
    end)

    -- Minimize / restore: semua slot emot & animasi TETAP ADA, cuma wheel yang disembunyikan
    xrzMinBtn.MouseButton1Click:Connect(function()
        local ok, _ = pcall(function()
            CoreGui.RobloxGui.EmotesMenu.Children.Main.EmotesWheel.Visible = false
        end)
        -- Sembunyikan frame UI tapi JANGAN hapus
        if UI.Top then UI.Top.Visible = false end
        if UI.Under then UI.Under.Visible = false end
        if UI.EmoteWalkButton then UI.EmoteWalkButton.Visible = false end
        if UI.Favorite then UI.Favorite.Visible = false end
        if UI.SpeedEmote then UI.SpeedEmote.Visible = false end
        if UI.SpeedBox then UI.SpeedBox.Visible = false end
        if UI.Changepage then UI.Changepage.Visible = false end
        if UI.Reload then UI.Reload.Visible = false end
        xrzTitle.Visible = false
        xrzMiniBar.Visible = true
    end)
    xrzRestoreBtn.MouseButton1Click:Connect(function()
        -- Restore semua â€” emot & animasi tetap di bagian masing-masing
        if UI.Top then UI.Top.Visible = true end
        if UI.Under then UI.Under.Visible = true end
        if UI.EmoteWalkButton then UI.EmoteWalkButton.Visible = true end
        if UI.Favorite then UI.Favorite.Visible = true end
        if UI.SpeedEmote then UI.SpeedEmote.Visible = true end
        if UI.SpeedBox then UI.SpeedBox.Visible = true end
        if UI.Changepage then UI.Changepage.Visible = true end
        if UI.Reload then UI.Reload.Visible = true end
        xrzTitle.Visible = true
        xrzMiniBar.Visible = false
    end)

    -- Resize toggle
    local xrzIsSmall = false
    xrzSizeBtn.MouseButton1Click:Connect(function()
        xrzIsSmall = not xrzIsSmall
        local scale = xrzIsSmall and 0.65 or 1
        local wheel = CoreGui.RobloxGui.EmotesMenu.Children.Main.EmotesWheel
        if wheel then
            wheel.Size = UDim2.new(0.35 * scale, 0, 0.65 * scale, 0)
        end
        xrzSizeBtn.Text = xrzIsSmall and "âŠž" or "âŠ¡"
    end)
    -- ===== END xrex zob =====
...
applyAnimation = function(animationData)
    ...
    if not animate or not humanoid then
        getgenv().Notify({
            ...
            Content = 'âŒ Animate or Humanoid not found',
            Duration = 3
        })
        return
    end
    ...
    if index == 1 and randomActive then
        ...
        if State.customAnimationEditorActive then
            ...
            if State.CustomAnimations.Sets[State.currentCustomAnimationName] and cat and name then
                ...
                State.SaveCustomAnimations(State.CustomAnimations)
                getgenv().Notify({ Title = "7yd7 | Saved", Content = "âœ… Saved " .. name, Duration = 3 })
                if State.RefreshCustomAnimUI then State.RefreshCustomAnimUI() end
                if refreshCustomAnimationState then refreshCustomAnimationState(true) end
                State.exitCustomAnimationEditor()
            end
...
function fetchAllEmotes()
    ...
    getgenv().Notify({
        Title = 'xrex zob',
        Content = "ðŸŽ‰ Loaded Successfully! Total Emotes: " .. State.totalEmotesLoaded,
        Duration = 5
    })
...
enterHUDEditor = function()
    ...
    propCorner.CornerRadius = UDim.new(0, 0)
    ...
    exportCorner.CornerRadius = UDim.new(0, 0)
    ...
    importCorner.CornerRadius = UDim.new(0, 0)
    ...
    resetCorner.CornerRadius = UDim.new(0, 0)
    ...
    lockCorner.CornerRadius = UDim.new(0, 0)
    ...
    addCorner.CornerRadius = UDim.new(0, 0)
    ...
    backCorner.CornerRadius = UDim.new(0, 0)
    ...
    table.insert(HUD.Connections, lockBtn.MouseButton1Click:Connect(function()
        ...
        getgenv().Notify({ 
            ...
            Content = HUD.IsUnlocked and "ðŸ”“ Interior Unlocked! Children are now editable." or "ðŸ”’ Interior Locked! Top-level only.", 
            Duration = 2 
        })
    end))
    ...
    table.insert(HUD.Connections, exportBtn.MouseButton1Click:Connect(function()
        ...
        setclipboard(HttpService:JSONEncode(data))
        getgenv().Notify({ Title = "7yd7 | HUD Editor", Content = "âœ… HUD settings copied", Duration = 2 })
    end))
    ...
    table.insert(HUD.Connections, importBtn.MouseButton1Click:Connect(function()
        ...
        imp.MouseButton1Click:Connect(function()
            ...
            if s and type(d) == "table" then
                ...
                popup:Destroy()
                getgenv().Notify({ Title = "7yd7 | HUD Editor", Content = "âœ… HUD settings imported", Duration = 2 })
            else
                getgenv().Notify({ Title = "Error", Content = "Invalid HUD JSON", Duration = 3 })
            end
        end)
    ...
    table.insert(HUD.Connections, resetBtn.MouseButton1Click:Connect(function()
        ...
        getgenv().Notify({ Title = "7yd7 | HUD Editor", Content = "ðŸ”„ All designs and frames have been fully reset", Duration = 3 })
    end))
    ...
    local function createPropRow(label, lOrder, isLarge)
        ...
        tbox.ZIndex = 6007
        local tc = Instance.new("UICorner"); tc.CornerRadius = UDim.new(0, 0); tc.Parent = tbox
    ...
    local delCorner = Instance.new("UICorner"); delCorner.CornerRadius = UDim.new(0, 0); delCorner.Parent = deleteBtn
    ...
    table.insert(HUD.Connections, deleteBtn.MouseButton1Click:Connect(function()
        ...
        ApplyUIVisibility()
        pcall(function() updateGUIColors() end)
        getgenv().Notify({ Title = "7yd7 | HUD Editor", Content = "ðŸ—‘ï¸ Custom Frame deleted", Duration = 2 })
    end))
    ...
    table.insert(HUD.Connections, addBtn.MouseButton1Click:Connect(function()
        ...
        getgenv().Notify({ Title = "7yd7 | HUD Editor", Content = "âž• Custom Frame added!", Duration = 2 })
    end))

    getgenv().Notify({ Title = "7yd7 | HUD Editor", Content = "âœï¸ Drag elements to reposition", Duration = 5 })
end
